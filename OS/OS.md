...
1. 管道命令仅处理`stdout`，忽略标准错误输出`stderr`。
2. 管道右侧的命令必须能够接受`stdin`。
3. 多个管道命令可以串联使用。
...
| :--------------: | :--------: | :----------------------------------------: | :----------------------------: |
| 标准输入`stdin`  |     0      |    Unix/Linux程序默认从`stdin`读取数据     |           连接到键盘           |
| 标准输出`stdout` |     1      |    Unix/Linux程序默认向`stdout`输出数据    |          显示在屏幕上          |
| 标准错误`stderr` |     2      | Unix/Linux程序默认向`stderr`流写入错误信息 | 显示在屏幕上，但与`stdout`分开 |
...
+ 使用文件描述符进行重定向：
  + `n >file`：将文件描述符n的输出重定向到file中。这里n可以是`0(stdin)`、`1(stdout)`、`2(stderr)`**之外**的任何文件描述符，但通常用于自定义的文件描述符。
